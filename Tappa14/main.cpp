...
// window
const char* window_title = "14 - small refactoring (cleanup)";
...
struct Enemy
{
    ...
    Enemy();
    void fire();
    void draw(sf::RenderWindow& window, sf::Texture& tex, sf::Texture& bullet_tex);
};
...
struct State
{
    ...
    State() : spaceship(), 
...

              {
                ...
                if (!enemy_texture.loadFromMemory(enemy_png, enemy_png_len) ||
                    !enemy_bullet_texture.loadFromMemory(enemy_bullets_png, enemy_bullets_png_len))
                {
                    std::cout << "enemy textures load failed\n";
                }
...
void Spaceship::draw (sf::RenderWindow& window)
{
    ...
    for(size_t i = 0; i < bullets.size(); ++i)
    {
        ...
        float offset_x = bullets[i].pos.x - 30.0; // (64 - 4) / 2
        float offset_y = bullets[i].pos.y - 24.5; // (64 - 15) / 2
...
void Enemy::draw(sf::RenderWindow& window, sf::Texture& tex, sf::Texture& bullet_tex)
{
    if(isAlive)
    {
        ...
        sf::Sprite bSprite(bullet_tex);
        bSprite.setScale({2.0, 2.0});
...
void State::draw (sf::RenderWindow& window)
{
    ...
    {
        enemies[i].draw(window, enemy_texture, enemy_bullet_texture);
    }
    ...
    for (size_t i = 0; i < spaceship.bullets.size(); )
    {
        ...
        for (int e = 0; e < enemies_per_wave; e++)
        {
            ...
            if (bulletRect.findIntersection(enemyRect))
            {
                ...
                active_enemies--;
                score += 100; // for every kills +100
                hit = true;
                hitSound.play();
                explosions.push_back({enemies[e].pos, 0.0, 0});
                break;
            }
        }
    ...
    switch(key.scancode)
    {
        ...
        case sf::Keyboard::Scancode::Space:
            if (state.spaceship.fire_timer >= state.spaceship.fire_rate)
            {
                ...
                state.spaceship.fire_timer = 0.0; 
...
int main()
{
    ...
    gameOverText.setPosition({240.0, 200.0});

    sf::Text restartText{state.font, "Press R to Restart", 30};
    restartText.setFillColor(sf::Color::White);
    restartText.setPosition({245.0, 290.0});

    sf::Text winText{state.font, "YOU WON!", 60};
    winText.setFillColor(sf::Color::Green);
    winText.setPosition({260.0, 200.0});

    sf::Text scoreText{state.font, "SCORE: 0", 24};
    scoreText.setFillColor(sf::Color::White);
    scoreText.setPosition({340.0, 10.0});

    sf::Text enemyCounterText{state.font, "ENEMIES: 0", 24};
    enemyCounterText.setFillColor(sf::Color::White);
    enemyCounterText.setPosition({600.0, 10.0});
    ...
    while (window.isOpen())
    {
        ...
        enemyCounterText.setString("ENEMIES: " + std::to_string(total_remaining));
